...
export function TierBadge({ tier, enchantment = 0 }: { tier: number | null; enchantment?: number }) {
  return (
    <span
      className={`figure rounded-sm px-1.5 py-px font-bold text-micro text-ink ${tierBg(tier)}`}
    >
      T{tier ?? "?"}.{enchantment}
    </span>
  );
...
export function QualityBadge({ quality }: { quality: number }) {
  ...
  return (
    <span
      className={`figure rounded-sm border px-1.5 py-px font-bold text-micro ${
  ...
  return (
    <span
      className={`inline-flex items-center gap-[5px] ${
        alternate ? "rounded-sm bg-warn/10 px-0.5 font-semibold text-warn" : ""
      } ${className}`}
    >
      <span
        aria-hidden
...
export function SpreadWarning({
  ...
  return (
    <span
      ...
      className="figure shrink-0 rounded-sm border border-warn/40 bg-warn/10 px-1.5 py-px font-bold text-micro text-warn"
    >
      ⚠ {cities} cidades
    </span>
  );
...
export function ZoneTag({ zone, label }: { zone: string; label: string }) {
  if (zone === "MESMA_CIDADE") {
    return (
      <span className="figure rounded-sm border border-line bg-raised px-1.5 py-px text-micro text-dim">
        sem viagem
      </span>
    );
  }
...
export function ReturnTag({
  ...
  if (isBestCity) {
    return (
      <span
        ...
        className="figure inline-flex items-center gap-[4px] rounded-sm border border-up/40 bg-up-dim px-1.5 py-px text-micro text-up"
      >
        ↩ {pct} · melhor cidade
      </span>
    );
  ...
  if (!mappingKnown || bestCityName === null || delta === null) {
    return (
      <span
        ...
        className="figure inline-flex items-center gap-[4px] rounded-sm border border-line bg-raised px-1.5 py-px text-micro text-muted"
      >
  ...
  return (
    <span
      ...
      className="figure inline-flex items-center gap-[4px] rounded-sm border border-warn/40 bg-warn/10 px-1.5 py-px text-micro text-warn"
    >
